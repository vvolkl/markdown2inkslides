...
def svg_open(self,filename):
    doc_width = self.width #inkex.unittouu(self.document.getroot().get('width'))
    doc_height = self.height #index.unittouu(self.document.getroot().get('height'))
...
def latex_effect(self, formula, packages):
    ...
    add_header = parse_pkgs(packages)
    create_equation_tex(latex_file, formula, add_header)
    ...
    try:
        ...
        #print >>sys.stderr, formula
        #print >>sys.stderr, "temporary files were left in:", base_dir
        sys.exit(1)

    os.system('dvips -q -f -E -D 600 -y 5000 -o "%s" "%s" > /dev/null 2> /dev/null' % (ps_file, dvi_file))
